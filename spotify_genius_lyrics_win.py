...
class SpotifyLyricsFetcher:
    ...
    def process_metadata_file(self, input_file, output_file, artist_column='Artist', track_column='Song title'):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if (idx + 1) % 10 == 0 or (idx + 1) == total:
                    ...
                    pd.DataFrame(results).to_csv(temp_file, index=False, encoding='utf-8', 
                                                  quoting=1, escapechar='\\', doublequote=True)  # quoting=1使用引号包围所有非数值字段
            ...
            # 最终保存结果使用更安全的参数
            results_df.to_csv(output_file, index=False, encoding='utf-8', quoting=1, escapechar='\\', doublequote=True)  # 使用引号包围字段并对特殊字符进行转义
...
def main():
    # 单独控制每个年份文件的处理
    # 设置为True表示处理该年份的文件，False表示不处理
    PROCESS_2013 = False
    PROCESS_2014 = True
    PROCESS_2015 = False
    
    # 如果上面三个都是False，则处理所有文件
    ...
    metadata_dir = r"d:\DESKTOP\auckland uni\CompSci 760\msc_emo_pred\datasets\DEAM\metadata"
    ...
    # 如果没有年份被选中且PROCESS_ALL_IF_NONE_SELECTED为True，则处理所有文件
    if not any_year_selected and PROCESS_ALL_IF_NONE_SELECTED:
        ...
        if PROCESS_2014:
            files_to_process.append("metadata_2014_cleaned.csv")  # 使用清理后的新文件
        if PROCESS_2015:
